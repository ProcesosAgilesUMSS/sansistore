...
export default function RegisterUserModal({
	...
	const handleInternalPhoneChange = (value: string) => {
		...
		if (errors.internalPhone)
			setErrors((prev) => ({ ...prev, internalPhone: "" }));
		if (serverError) setServerError("");
	};
	...
	const validate = (): boolean => {
		...
		if (!trimmedName) {
			...
			newErrors.displayName =
				"El nombre es demasiado corto (mín. 3 caracteres).";
		} else if (/\s{3,}/.test(displayName)) {
			newErrors.displayName =
				"Error: El formato de nombre no permite espacios excesivos.";
		}
		...
		if (internalPhone && !/^\d+$/.test(internalPhone)) {
			newErrors.internalPhone =
				"El teléfono interno debe contener solo números.";
		}
              ...
              `}
					...
					<div>
						...
						<div className="grid grid-cols-2 gap-2">
							{SELECTABLE_ROLES.map((role) => {
								...
								const isDisabled =
									isLoading || (!isSelected && selectedRoles.length >= 2);
                    ...
                    `}
									...
									>
										...
										<span
											style={{
												color: isSelected ? colors.text : "var(--theme-text)",
											}}
										>
											{ROLE_LABELS[role]}
										</span>
