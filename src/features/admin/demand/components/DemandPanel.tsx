...
const barColor = (count: number, max: number): string => {
  if (max === 0) return '#B4B2A9';
  const ratio = count / max;
  if (ratio >= 0.6) return '#88B04B';
  if (ratio <= 0.15) return '#B4B2A9';
  return '#5B8DB8';
};
    ...
    <div className="max-w-5xl">
      ...
      {summary && !loading && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
          <div className="bg-[var(--theme-secondary-bg)] rounded-xl px-4 py-3">
            <p className="text-[9px] text-[var(--theme-text)]/40 uppercase tracking-wide mb-1.5">Hora pico</p>
          ...
          <div className="bg-[var(--theme-secondary-bg)] rounded-xl px-4 py-3">
            <p className="text-[9px] text-[var(--theme-text)]/40 uppercase tracking-wide mb-1.5">Total pedidos</p>
            <p className="text-[22px] font-semibold text-[var(--theme-text)] leading-none">{summary.totalOrders}</p>
          </div>
          <div className="bg-[var(--theme-secondary-bg)] rounded-xl px-4 py-3">
            <p className="text-[9px] text-[var(--theme-text)]/40 uppercase tracking-wide mb-1.5">Promedio / hora</p>
            <p className="text-[22px] font-semibold text-blue-600 leading-none">{summary.avgPerHour}</p>
          </div>
          <div className="bg-[var(--theme-secondary-bg)] rounded-xl px-4 py-3">
            <p className="text-[9px] text-[var(--theme-text)]/40 uppercase tracking-wide mb-1.5">Hora más baja</p>
            <p className="text-[16px] font-semibold text-amber-600 leading-none">
      ...
      {loading && (
        <div className="flex flex-col gap-2">
          {[1, 2, 3].map((i) => (
            <div key={i} className="h-20 bg-[var(--theme-secondary-bg)] rounded-xl animate-pulse" />
          ))}
        </div>
      )}
      ...
      {!loading && summary && summary.totalOrders > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          {/* Histograma */}
          <div className="md:col-span-2 bg-[var(--theme-secondary-bg)] rounded-xl p-5">
            <p className="text-[13px] font-semibold text-[var(--theme-text)] mb-1">
              Pedidos por franja horaria
            </p>
            <p className="text-[10px] text-[var(--theme-text)]/40 mb-4">
              {startDate} — {endDate}
            </p>
            ...
            <div className="flex gap-4 mt-2">
              <div className="flex items-center gap-1.5">
                <div className="w-2.5 h-2.5 rounded-sm" style={{ background: '#88B04B' }} />
                <span className="text-[10px] text-[var(--theme-text)]/50">Alta demanda</span>
              </div>
              <div className="flex items-center gap-1.5">
                <div className="w-2.5 h-2.5 rounded-sm" style={{ background: '#5B8DB8' }} />
                <span className="text-[10px] text-[var(--theme-text)]/50">Normal</span>
              </div>
              <div className="flex items-center gap-1.5">
                <div className="w-2.5 h-2.5 rounded-sm" style={{ background: '#B4B2A9' }} />
                <span className="text-[10px] text-[var(--theme-text)]/50">Baja demanda</span>
              </div>
            </div>
          </div>

          {/* Top 5 horas pico */}
          <div className="bg-[var(--theme-secondary-bg)] rounded-xl p-5">
            <p className="text-[13px] font-semibold text-[var(--theme-text)] mb-1">
              Horas pico
            </p>
            <p className="text-[10px] text-[var(--theme-text)]/40 mb-4">
            ...
            <div className="flex flex-col gap-2">
              {summary.top5.map((item) => {
                ...
                return (
                  <div key={item.hour} className="bg-[var(--theme-bg)] rounded-lg px-3 py-2.5">
                    <div className="flex items-center justify-between mb-1.5">
                      <span className="text-[11px] font-semibold text-[var(--theme-text)]">
