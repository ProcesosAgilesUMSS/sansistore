...
import type { PaymentReconciliationItem, ReconciliationIssueType } from '../types';
...
function mapPayment(snapshot: QueryDocumentSnapshot<DocumentData>): PaymentRecord {
  const data = snapshot.data();
...
function buildReconciliationItems(
  ...
  return orders
    ...
    .map((order) => {
      const payment = paymentByOrderId.get(order.orderId) ?? paymentByOrderId.get(order.id);
      const issueType = getIssueType(order, payment);
      if (!issueType) return null;

      const paymentAmount = payment?.amount ?? null;
      const difference = Number((order.total - (paymentAmount ?? 0)).toFixed(2));
