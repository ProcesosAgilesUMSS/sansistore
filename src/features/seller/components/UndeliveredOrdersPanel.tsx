...
import { useUndeliveredOrderDecision } from "../hooks/useUndeliveredOrderDecision";
		...
		<article className="overflow-hidden rounded-3xl border border-(--theme-border) bg-linear-to-br from-(--theme-card-bg) to-(--theme-secondary-bg)/40 shadow-sm duration-200 hover:shadow-xl">
			<div className="p-5">
				...
				{(restartSuccess || cancelSuccess) && (
					<p
						className={`mt-4 text-sm font-700 ${restartSuccess ? "text-primary" : "text-red-600"}`}
					>
						{restartSuccess
...
export default function UndeliveredOrdersPanel({
	embedded = false,
}: {
	embedded?: boolean;
}) {
	...
	const {
		...
		restartOrder,
		cancelOrder,
	} = useUndeliveredOrderDecision();

	const [selectedOrder, setSelectedOrder] = useState<Order | null>(null);
	const [decisionOrder, setDecisionOrder] = useState<Order | null>(null);
	const [decisionMode, setDecisionMode] = useState<"restart" | "cancel" | null>(
		null,
	);
	...
	return (
		...
		>
			...
			{(error || decisionError) && (
				<ErrorMessage
					message={error || decisionError || "Ha ocurrido un error."}
				/>
			)}
			...
			<div className="grid w-full gap-6">
				<section className="w-full rounded-3xl p-5">
					...
					{loading ? (
						...
						<EmptyOrders description="No hay pedidos no entregados pendientes de decisión." />
					) : (
