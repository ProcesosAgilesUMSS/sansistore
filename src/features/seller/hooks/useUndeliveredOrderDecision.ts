...
import {
  cancelUndeliveredOrder,
  restartUndeliveredOrder,
} from '../services/undeliveredOrderDecisionService';

export function useUndeliveredOrderDecision() {
  ...
  const restartOrder = useCallback(
    async (orderId: string) => {
      ...
      try {
        await restartUndeliveredOrder(db, orderId, user.uid);
  ...
  const cancelOrder = useCallback(
    async (orderId: string) => {
      ...
      try {
        await cancelUndeliveredOrder(db, orderId, user.uid);
