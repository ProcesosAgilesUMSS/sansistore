...
export default function OrderDetailsPanel({
	...
	const canConfirmReception =
		isDeliveredOrder(order) && !order.buyerReceptionConfirmed;
						...
						{order.status === "CANCELADO" &&
							(order.incidentNotes || order.incidentReason) && (
								<div className="flex items-start gap-3 pt-4 border-t border-(--theme-border)">
									...
									<div>
										<h4 className="text-sm font-bold mb-1 text-red-500">
					...
					<div className={cardClass}>
						...
						{returnSuccess ? (
							...
							<p className="text-xs opacity-50 italic flex items-center gap-1.5">
								<Info size={14} /> El plazo para devolver este pedido ha
								expirado.
							</p>
						) : (
							<p className="text-xs opacity-50 italic flex items-center gap-1.5">
								<Info size={14} /> Las devoluciones solo están disponibles para
								pedidos entregados.
							</p>
						)}
					</div>
