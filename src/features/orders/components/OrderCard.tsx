...
const getStatusStyles = (status: string) => {
  switch (status) {
    case 'delivered':
      return 'bg-[#88B04B]/10 text-[#88B04B]'; // Color primario de la marca
    case 'in_transit':
...
export default function OrderCard({ order, onSelect }: OrderCardProps) {
  const formattedDate = order.createdAt?.toDate 
    ? order.createdAt.toDate().toLocaleDateString('es-BO', { day: 'numeric', month: 'short', year: 'numeric' })
    : 'Fecha no disponible';
      ...
      <div className="flex items-center justify-between sm:justify-end gap-4 border-t border-(--theme-border) sm:border-none pt-3 sm:pt-0">
        <div className="flex flex-col sm:text-right">
          ...
          <span className="text-lg font-display font-black text-primary">
            {order.total.toFixed(2)} <small className="text-xs font-normal">Bs.</small>
          </span>
        </div>
