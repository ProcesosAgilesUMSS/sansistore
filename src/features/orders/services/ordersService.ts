...
async function processQuerySnapshot(
	...
	await Promise.all([
		...Array.from(locationIds).map(async (id) => {
			...
			if (snap.exists())
				locationMap.set(id, (snap.data() as any).label || "Sin etiqueta");
		}),
	...
	const orders = await Promise.all(
		docs.map(async (orderDoc) => {
			...
			if (delData?.courierId) {
				courierName =
					userMap.get(delData.courierId)?.displayName || "Mensajero sin nombre";
			}
			...
			const items = (await Promise.all(
				itemsSnapshot.docs.map(async (itemDoc) => {
					...
					if (!stockMap.has(item.productId)) {
						...
						stockMap.set(
							item.productId,
							invSnap.exists() ? invSnap.data().stockAvailable || 0 : 0,
						);
					}
					return {
						itemId: itemDoc.id,
