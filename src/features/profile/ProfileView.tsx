...
export default function ProfileView() {
	...
	const showMisDirecciones =
		profile.roles.length === 0 || profile.roles.includes("comprador");
	const showMisPedidos =
		profile.roles.length > 0 &&
		profile.roles.some((r) => ["comprador", "admin"].includes(r));
	const showCalificacionMensajero =
		profile.roles.length > 0 &&
		profile.roles.some((r) => ["mensajero", "admin"].includes(r));
	const isMensajero =
		profile.roles.includes("mensajero") || profile.roles.includes("admin"); // ← NUEVO
					...
					{showMisDirecciones && (
						...
						>
							...
							<div className="flex flex-col text-left">
								...
								<span className="text-[11px] font-medium text-text-light/40 group-hover:text-primary/60 transition-colors">
									Gestionar entrega
								</span>
							</div>
						</a>
