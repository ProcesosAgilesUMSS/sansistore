...
async function requireAdmin(request: Request): Promise<AdminGuardResult> {
	...
	try {
		...
		const firestoreRoles = Array.isArray(adminData?.roles)
			? adminData.roles
			: [];
		...
		return {
			error: jsonResponse({ message: "Token de autenticacion invalido." }, 401),
		};
	}
}
...
function validateCreateUserPayload(payload: CreateUserRequest) {
	...
	if (!isAllowedInstitutionalEmail(email)) {
		return {
			error: "Solo se permiten dominios institucionales UMSS.",
		};
	}
...
export const POST: APIRoute = async ({ request }) => {
	try {
		...
		const { displayName, email, phoneNumber, ci, internalPhone, roles } =
			validation.data;
		...
		if (await ciExistsInFirestore(ci)) {
			return jsonResponse(
				{
					message:
						"Ya existe un usuario registrado con esta cédula de identidad.",
				},
				409,
			);
		}
...
export const PATCH: APIRoute = async ({ request }) => {
	try {
		...
		return jsonResponse({
			...
			user: serializeUser(uid, updatedData),
		});
	} catch (error) {
		if (error instanceof ExternalServiceError) {
			return jsonResponse({ message: error.message }, error.status);
