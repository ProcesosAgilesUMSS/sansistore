    ...
    const handleLogin = async () => {
        ...
        try {
            ...
            if (!isInstitutionalEmail(result.user.email)) {
                await signOut(auth);
                const errorMsg =
                    'Solo se permiten cuentas institucionales (@umss.edu) para acceder a SansiStore.';
                setError(errorMsg);
                onError?.(errorMsg);
                return;
            }
    ...
    return (
        <>
            <button
                onClick={handleLogin}
                disabled={loading}
                className={`inline-flex items-center justify-center gap-2.5 px-4 py-3 rounded-full border transition-all active:scale-[0.98] text-sm font-semibold text-(--theme-text) border-(--theme-border) bg-(--theme-card-bg) hover:border-primary hover:text-primary disabled:opacity-50 disabled:cursor-not-allowed ${className}`}
            >
            ...
            {error && (
                <div className="mt-2 text-xs text-(--theme-error)">{error}</div>
            )}
        </>
    );
}